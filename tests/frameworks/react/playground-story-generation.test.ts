...
describe('react story generation from playground-like props', () => {
  it('generates a TaskList story with correct file path and imports', () => {
    const story = generateStory({
      ...
      props: {
        ...
        children: {
          __isJSX: true,
          source:
            '<><TaskCard task={{ id: "1", title: "Review" }} onAction={() => {}} /><Button variant="secondary">Load more</Button></>',
    ...
    expect(story.filePath).toBe('/repo/playground/react/src/components/TaskList.stories.tsx')
    expect(story.storyName).toBe('Tasklistcaptured')

    expect(story.content).toContain("import { TaskList } from './TaskList';")
    expect(story.content).toContain("import { TaskCard } from './TaskCard.tsx';")
    expect(story.content).toContain("import { Button } from './Button.tsx';")
