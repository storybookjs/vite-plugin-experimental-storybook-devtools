...
export interface ReactDetectionSuiteOptions {
  /** Suite title, e.g. 'React playground detection coverage'. */
  suiteTitle: string
  /**
   * Substring the TaskList registry entry's filePath must contain. Hosts
   * differ: Vite reports the playground's own path, while rspack resolves
   * `playground/rsbuild/src` (a symlink) to the canonical
   * `playground/react/src` — so the guarantee asserted is "real source
   * file, not 'unknown'", with a host-appropriate prefix.
   */
...
/**
 * Detection-coverage suite for every playground rendering the canonical
 * `playground/react/src` tree (directly or via symlink). Framework- or
 * host-specific detection tests stay in the individual specs.
 */
export function registerReactDetectionSuite(
  test: TestLike,
  options: ReactDetectionSuiteOptions,
) {
  test.describe(options.suiteTitle, () => {
    ...
    test('uses real source metadata for TaskList (no unknown path)', async ({
      ...
      expect(meta?.filePath).toContain(options.taskListFilePath)
